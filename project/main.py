...
from signup import Search, Signup
        ...
        if request.method == "GET":
            ...
            try:
                ...
                if (patient is None) and (password == form.Re_password.data):
                    ...
                    db.session.commit()
                    value = (
                        f"user is {username} <br> UID is {NID} <br> "
                        f"mail is {email} <br> Fname is {Fname} <br> "
                        f"Lname is {Lname} <br> Birthday is {BD}"
                    )
                    return render_template(
                        "out.html", output=value, Statues="This User is added"
                    )
                else:
                    return render_template(
                        "signup.html", form=form, value="This id is already IN"
                    )
            except Exception as e:
    ...
    def search():
        ...
        if request.method == "GET":
            ...
            try:
                ...
                value = (
                    f"User is {patient.username} <br> UID is {patient.NID} <br> "
